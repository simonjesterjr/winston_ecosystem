...
async def list_tools() -> list[Tool]:
    tools = [
        ...
        Tool(
            name="wv2_get_mid_month_scoreboard",
            description=(
                "Fetch the Mid-month Scoreboard (MMS) JSON plus pdf_path / telegram_media_path. "
                "Generated by Wv2 Sidekiq at 06:00 America/Denver on the third Wednesday. "
                "Fetch only — never invent metrics. date is the scheduled Wednesday (YYYY-MM-DD)."
            ),
            inputSchema={
                "type": "object",
                "properties": {
                    "date": {
                        "type": ["string", "null"],
                        "description": "Scheduled third-Wednesday date YYYY-MM-DD. Omit for latest.",
                    },
                },
            },
        ),
...
async def call_tool(name: str, arguments: dict[str, Any]) -> list[TextContent]:
    ...
    try:
        ...
        elif name == "wv2_get_mid_month_scoreboard":
            params = _query_params(args)
            try:
                data = await _get(WV2_BASE, "/internal/mid_month_scoreboards", params=params, **hop_kw)
            except httpx.HTTPStatusError as exc:
                if exc.response.status_code == 422:
                    data = exc.response.json()
                else:
                    raise
            if data.get("telegram_media_path"):
                data = _attach_delivery_hints(data)
