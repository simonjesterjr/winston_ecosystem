...
16c. **wv2_get_mid_month_scoreboard**
    - Purpose: Fetch the **Mid-month Scoreboard (MMS)** — monthly business review PDF/JSON (journals, passed signals, PCS, period return/Sharpe). Generated by Wv2 `MidMonthScoreboardJob` at 06:00 America/Denver on the **third Wednesday**.
    - Inputs: `{ "date": "2026-08-19" }` (scheduled Wednesday; omit for latest).
    - Source: `GET /internal/mid_month_scoreboards`
    - Returns: payload with `summary`, `scores`, `findings`, `pdf_path`, `telegram_media_path`. 422 `mms_not_ready` if not generated.
    - Skill: `winston-mms`
    - Non-goals: triggering generation from Telegram; inventing metrics when the file is missing.
...
- `wv2_market_snapshot` — shuffle the active-portfolio market population, live-evaluate via Sidekiq until 3 non-quiet names (testing / breach_up / breach_down) or the list is exhausted; live internet price vs prior EOD close + atr_17. Class-share aliases (BRK.B → BRK-B) are canonicalized. Rows/summary flag `previous_close_stale` when the parquet bar is more than one weekday session old. Focusing tool; quiet names are not listed.
