...
**Session Order Slate**:
The stop-market **Order Intents** parked before the session for one **Active** **Operational Portfolio** — initial entry, pyramid scale-in, and protective **Working Stop**. Stops are always **stop-market** (never limit). Entries and pyramids are **DAY** (rebuild each night; unfilled die at the close). Protective stops are **GTC** and **replaced**, never cancel-all. Live Trend Following target once write exists; next-open remains the lab default until a **TradingStrategy** is fingerprinted on resting-touch.
