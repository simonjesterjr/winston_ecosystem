...
**Slate Approve**:
The Human-Gated lock of one night’s **Session Order Slate** on a paper Interactive Brokers-bound Trend Following **Operational Portfolio** (Walnut first): Approve the package; remaining ready legs **Desk Send** one stop-market **Order Intent** at a time (WQ-shaped). Distinct from **Plan Approve** (WQ Monday book) and from **Slate Automation** (policy-send, no click).
_Avoid_: basket send of the whole slate; DA place_order; mixing with WQ Plan Approve
...
**Slate Automation**:
An explicit flag on an **Operational Portfolio** together with its **TradingStrategy** fingerprint that opts that pair into mechanical **Session Order Slate** instructions (Turtle **Slate Contest**, **Moment of Truth** sizing) and **policy-automatic Desk Send** of that slate each session. Distinct from **Active**, **Execution Mode**, **Slate Approve**, and from Daily Analysis opening **Positions**. Walnut paper is **Slate Approve** + per-leg Send, not this flag. A human may halt or **Desk Pass** before the session; the Daily Activity Report is a review, not a send click. Winston Quiver stays **Plan Approve**. Whole-slate accept-fill (one click books every fill) stays glossary aspiration.
...
- **Confirm vs Send** — resolved (Grill B Q1) for Trend Following and live: **Desk Confirm** = book only; **Desk Send** = place **Order Intent** only (no auto-open Position). **Carve-out (Grill 2026-09-06):** on the Interactive Brokers **paper**-bound **WQ Shadow Portfolio**, the tracking-task Confirm click **is** **Desk Send** of that one **market** **Order Intent**; booking is **Accept-Fill** of the matched print. **Walnut paper (Grill 2026-09-09):** wiring-proof Confirm remains one DAY MKT; the intended loop is **Slate Approve** then per-leg Confirm = **Desk Send** of one **stop-market** **Order Intent**; DUT print **Accept-Fills**. Mint stays dummy_sim book-only. Not **Slate Automation**. Paper DUT `order_write` is **ADR-013**. Live Schwab remains off.
