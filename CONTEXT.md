...
**Mode D (fulfillment)**:
A portfolio fulfillment choice beside Mode C, not a fourth Plan A / Plan B / Plan C rung. The book trades the underlying long and short. Stored as `fulfillment_packaging_policy.fulfillment_mode=mode_d` with `leap_fulfillment=none`. The entry is stock only. After that stock print books, the desk offers a covered call on each open long of at least 100 shares (`floor(shares/100)` contracts). The call is opt-in. A short is never offered one. The stock lot and its short call unwind together: buy the call back, then sell the stock. A filled call does not replace the protective stop and does not clear a naked-stop flag. User acceptance uses the same Ops Shell desk as every other book, on the paper account that will actually hold the shares. See `docs/business-context/mode-d-ops-shell-uat.md`.
