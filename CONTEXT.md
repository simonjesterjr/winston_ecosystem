...
**Working Stop**:
The stop price Winston treats as current on an open **Position** on the **Booked Capital Spine** (`updated_stop` / desk-updated). Turtle S2 (Walnut): while that name is **under max lots**, 20-day is **not evaluated**; all lots share last-entry **2N** (GTC, `move_to_last_entry` on a pyramid fill) and a DAY add at last fill ± the TS pyramid step. Once **maxed**, 20-day is evaluated; when it has **passed** 2N in the trade’s favor the working stop **is** the 20-day (may ratchet, even through last purchase). One broker stop; GTC replaced, never cancel-all.
_Avoid_: 20-day while still adding; tighter-of-2N-and-20-day as always-on; two live protective stops; assuming Daily Analysis already implements this switch; treating a missing broker stop as fine when the **Protective Stop Guardrail** applies
