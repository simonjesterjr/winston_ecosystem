...
**Plan Approve**:
The Human-Gated verb for a **Monday Rebalance Plan**: Approve or Reject the whole package (optional skip-line with reason). After Approve, the plan is **locked** (Approve disabled). Remaining ready legs run **one at a time** (exits, then rebalances, then enters): dummy_sim Confirm still books the lot; on an Interactive Brokers **paper**-bound **WQ Shadow Portfolio**, that Confirm is **Desk Send** of a market order and the journal stays **working** until matched fill evidence **Accept-Fills** it at the print. A name that cannot resolve a fill is a per-leg **HITL** flag — the rest of the plan stays approved. Live WQ Schwab write is after test cycles.
_Avoid_: silent book-from-PDF; auto-booking the whole package on Approve; treating missing fill as a plan-wide fail; auto-execute on Mint or Execution Mode real; booking last-close at the IBKR-paper Confirm click; hiding Confirm until the cash session opens
...
_Avoid_: treating working as booked cash or a closed lot; hiding Confirm until the cash session opens; a basket of working orders from one click
...
The canonical parquet format DM produces and consumers read. OHLCV + baked-in derivatives (ATR-17, MACD 12/26/9 as `macd_line`/`macd_signal`/`macd_histogram`, supported MAs). Consumers must read `atr_17` from parquet; missing column triggers portfolio skip (`missing_data`).
...
**Human-Gated**:
Position open/close (and free-form desk fills) require an explicit operator desk action — ops form, ops shell, or Cromwell/MCP on a human instruction — not silent mutation by **Daily Analysis** or by inbound broker confirmation evidence alone. **Execution Mode** `real` is always human-gated for **booking entries and pyramids** unless a named Accept-Fill carve-out already **Desk Sent** that command. **Slate Automation** may policy-**Desk Send** the **Session Order Slate**. On an Interactive Brokers **paper**-bound **WQ Shadow Portfolio**, tracking Confirm is the gate that **Desk Sends** one approved Monday-plan market leg. On paper **Portfolio Walnut**, **Slate Approve** **Desk Sends** remaining ready stop-market **Order Intents** automatically (one at a time, protective first); the DUT print **Accept-Fills** (IBKR is fulfillment SoT). L1 evidence may prefill or surface mismatch; unmatched prints still need **Desk Confirm** or **Corrective Amend**.
_Avoid_: treating draft creation as a fill, silent book-from-email, equating policy Send with auto-open Position, treating an IBKR bind alone as **Slate Automation**
...
**Desk Confirm**:
Book on the **Booked Capital Spine** for a **Single Fulfillment Identity** (draft → executed **Journal** / open or closed lot). Human already has a fill story (traded outside Winston, accepted a prefilled **Trade Notification**, or dummy_sim paper path). Does **not** place a broker order except the **WQ Shadow Portfolio** Interactive Brokers **paper** carve-out (tracking Confirm = **Desk Send** of that one market order; booking waits for **Accept-Fill** at the print).
_Avoid_: silent book-from-API; treating Mint/TF Confirm as Send; booking WQ last-close at the IBKR-paper click
...
_Avoid_: overloaded Confirm-that-sends, DA place_order, auto-send pyramids on a non-enabled fingerprint, treating Active or paper as automation, equating policy Send with Desk Confirm, hard-coding market-only on the Interactive Brokers adapter because Winston Quiver uses market

**Order Intent**:
The broker instruction **Winston v2** asks **Broker Gateway** to place: side, size, **Order Intent** type (market, limit, stop-market, …), time-in-force, and optional limit/stop prices. Type is **Fulfillment Packaging Policy** on the **Operational Portfolio**, not an adapter-class constant. Winston Quiver paper policy is regular-hours **market**; a paper Mint / Trend Following bind on the same Interactive Brokers adapter uses **Session Order Slate** stop-market (and later limit) intents.
_Avoid_: one IBKR adapter that can only market; treating WQ market-only as law for Mint stops, exits, and pyramids

**Session Order Slate**:
The stop-market **Order Intents** parked before the session for one **Active** **Operational Portfolio** — initial entry, pyramid scale-in, and protective **Working Stop**. Stops are always **stop-market** (never limit). Entries and pyramids are **DAY** (rebuild each night; unfilled die at the close). Protective stops are **GTC** and **replaced**, never cancel-all. On paper Interactive Brokers Trend Following (Walnut first), the overnight **Rebuild** is the daily evaluative step — same kickoff as **Daily Analysis** (`DailyAnalysisJob` after **SessionDataGate**, including DM `data_ready` catchup). Rebuild snapshots DUT first: aligned GTC is not redrawn; DAY already gone after the close is expired in Winston without a cancel of a missing id; a re-click is idempotent when the draft already matches last bar + DUT. **Slate Approve** stays the human gate (not **Slate Automation**). Live Trend Following target once write exists; next-open remains the lab default until a **TradingStrategy** is fingerprinted on resting-touch.
_Avoid_: mixing next-open fills with a fill-relative stop from the unfilled breakout; cancel-all of live protective stops; treating the slate as Daily Analysis opening Positions; buy/sell limits as breakout entries

**Slate Approve**:
The Human-Gated lock of one night’s **Session Order Slate** on a paper Interactive Brokers-bound Trend Following **Operational Portfolio** (Walnut first): Approve the package; remaining ready legs **Desk Send** one stop-market **Order Intent** at a time automatically (protective first). Distinct from **Plan Approve** (WQ Monday book) and from **Slate Automation** (policy-send with **no** Approve click). Overnight Rebuild is automatic after session bars land (same job as Daily Analysis); the button remains an idempotent initiator. Rebuild cancels leftover Walnut **DAY** entry/pyramid tickets still on DUT; **GTC** protective stops stay (never cancel-all, never redraw when DUT/Winston already agree).
_Avoid_: basket HTTP of the whole slate; DA place_order; mixing with WQ Plan Approve; cancelling live protective GTC on rebuild

**Fill-Driven Repark**:
After a Desk-Sent Session Order Slate stop-market **Accept-Fills** at the Interactive Brokers paper DUT print, Winston v2 mints a **delta** slate (not a full nightly rebuild): Good-Til-Cancelled protective stop at last fill ± the Operational Portfolio’s **TradingStrategy** Average True Range multiplier, plus a DAY pyramid at last fill ± the TS pyramid step when lots remain under max. Opposite DAY entry on the filled name is cancelled (first-to-touch). Full rebuild is forbidden during the cash session — it would cancel unrelated DAY parks. Operator **Slate Approve**s the delta; remaining legs auto-send (protective first). Not **Slate Automation**. Broker push/websocket is a later evaluation; poll cadence is 15 minutes.
_Avoid_: calling Builder/DayTeardown intra-day; copying the entry trigger onto the Working Stop; hard-coding 0.5N / 2N; skipping Approve; treating poll lag as a live-tape defect
...
**Slate Automation**:
An explicit flag on an **Operational Portfolio** together with its **TradingStrategy** fingerprint that opts that pair into mechanical **Session Order Slate** instructions (Turtle **Slate Contest**, **Moment of Truth** sizing) and **policy-automatic Desk Send** of that slate each session. Distinct from **Active**, **Execution Mode**, **Slate Approve**, and from Daily Analysis opening **Positions**. Walnut paper is **Slate Approve** then automatic Send of remaining ready legs, not this flag. A human may halt or **Desk Pass** before the session; the Daily Activity Report is a review, not a send click. Winston Quiver stays **Plan Approve**. Whole-slate accept-fill (one click books every fill) stays glossary aspiration.
...
**Accept-Fill**:
Policy that books a matched **Trade Notification** onto the **Booked Capital Spine** without a second human Confirm click. On Interactive Brokers **paper** DUT, a command already **Desk Sent** (WQ market leg, Walnut MKT wiring proof, or Walnut stop-market slate leg) **Accept-Fills** at the DUT print — entries, pyramids, and protective stops — because the broker is fulfillment SoT. Unmatched / reject / cancel / orphan stay attention; do not invent a book. Dummy_sim and Manual stay Confirm. Whole-slate accept-fill (one click books every fill) is not scheduled.
_Avoid_: silent invent of unmatched prints; treating broker NLV movement as a booked lot; booking last-close when the DUT print is still outstanding; assuming tickle is unattended login
...
- **`interactive_broker_trader_api`** — Interactive Brokers Web API / Client Portal Gateway; fixtures by default; live GET gated by `BG_IBKR_LIVE_READ`; CPGW **tickle** keep-alive; write off at L1 until the fulfillment-write ADR. Paper vs live are **bindings** (`env=sandbox` DUT paper vs `env=live`), **same adapter class**. That class must carry **market, limit, and stop-market** **Order Intents**; Winston Quiver paper using market does not strip those types from a later paper Mint bind. WQ talks only to Broker Gateway. Dummy_sim remains Winston-internal paper.  
Paper may still choose `manual` explicitly; product default for paper intake practice is **`dummy_sim` through BG**.

**Fulfillment Label**:
Operator-facing short name of how an **Operational Portfolio** fulfills, shown **beside** **Execution Mode** (not instead of it): `{Vendor} {Nickname}` from the bound **Adapter Binding** (e.g. `IBKR DUT070450`, `IBKR UT`, `Schwab` last-4). dummy_sim and manual have **no extra chip** — Execution Mode is enough. Glance surfaces: Active OPs, positions/pending headers, WQ header and Tracking OP broker row, portfolio live-eval, **Daily Activity Report**, Telegram. Distinct from **Execution Mode**, **Capital Authority**, and **Fulfillment Packaging Policy**. Never a full live account number on ops surfaces, DAR, or Telegram.
_Avoid_: replacing paper/real with the broker name; dumping full live account numbers; treating the label as automation or as Risk Capital; pasting **Fulfillment Ritual** steps onto the chip, DAR, or Telegram; DAR/Telegram `needs login` attention (that glance lives on the ops shell and WQ)

**Adapter Binding**:
Broker Gateway account-level instance of a fulfillment adapter — vendor account, `env` sandbox|live, capabilities, and the nickname used in the **Fulfillment Label**. Winston v2 associates an **Operational Portfolio** via opaque `broker_binding_id`. Paper vs live are bindings of the **same** adapter class. Multi-OP on one account (Q8) is deferred.
...
**Working Stop**:
The stop price Winston treats as current on an open **Position** on the **Booked Capital Spine** (`updated_stop` / desk-updated). Same Winston / Trend Following / Donchian / 2N law in **Daily Analysis**, WUT, and live — paper vs live vs **Session Order Slate** differs only in **Fulfillment**. Turtle S2 (Walnut): while that name is **under max lots**, 20-day is **not evaluated**; all lots share last-entry **2N** (GTC, `move_to_last_entry` on a pyramid fill) and a DAY add at last fill ± the TS pyramid step. Once **maxed**, 20-day is watched; Working Stop stays 2N until 20-day has **passed** 2N in the trade’s favor, then Working Stop **is** the 20-day (may ratchet, even through last purchase). Pierce = **signaled** stop-out flatten-all. GTC fill: touch → stop, gap → session open, same session. One Working Stop; GTC replaced, never cancel-all.
_Avoid_: 20-day while still adding; tighter-of-2N-and-20-day as always-on; two live protective stops; DA skipping 2N on paper HITL; dummy-sim Confirm on a live GTC; treating a missing broker stop as fine when the **Protective Stop Guardrail** applies

**Protective Stop Guardrail**:
Every open **Position** that the methodology gives a **Working Stop** must have a matching protective order **in the market** when the bound adapter can park one (Interactive Brokers stock/ETF: GTC stop-market). Evaluated continuously against the broker book — a naked lot is attention, not optional. **Extra-Modal Fulfillment** (LEAPs, options, related instruments): the stop is evaluated on the **signal Market**; a trigger is a **HITL** to sell the related fill, not a silent option-stop unless packaging says so. Not applicable where the recipe has no stop (e.g. some Winston Quiver lots).
_Avoid_: cancel-all that leaves lots naked; assuming an option contract carries the same STP as the underlying; skipping the check because tickle failed (that is its own attention)

**Stop-Out Reconciliation**:
Desk workflow when a **Working Stop** is hit (or life exits a lot that should have been stopped): book an exit **Journal** **required-linked** to the open **Position**, storing a snapshot (working_stop_at_exit, fill_price, gap) with reason (e.g. external_stop). **Warn** (do not hard-block by default) when fill diverges from **Working Stop**. **Fulfillment** is the only mode split: paper **dummy_sim** — **Daily Analysis** mints a **signaled** stop-out and Desk Confirm books at the simulated GTC fill (touch → stop, gap → open, same session); **Session Order Slate** (Walnut) — same Working Stop, GTC stop-market parked, **Accept-Fill** at the DUT print is SoT (do not dummy-sim Confirm on a live GTC); live — broker print. Under **Slate Automation** in discovery, a matched protective-stop **Trade Notification** **Accept-Fills** this path so **Unit Heat** is not left holding a ghost lot.
_Avoid_: ad-hoc exit with no position/stop provenance; treating a methodology stop-out as unsignaled; dummy-sim Confirm on a parked GTC; silent large gaps; leaving the lot open after a parked stop printed
...
Exits **may** be booked without a Winston exit signal when Winston **did not compute** the close — broker/clearing misses, downstream errors, discretionary flatten, a print the engine never saw — with reason codes and, when closing a known lot, position linkage (**Stop-Out Reconciliation** when stop-related). A **Working Stop** pierce **is** a methodology signal: **Daily Analysis** evaluates it (paper HITL mints a **signaled** stop-out). This allowance is **not** “DA may skip 2N on paper.” Asymmetry remains: entries are methodology-gated; unsignaled closes stay honest about market reality Winston did not see.
_Avoid_: requiring a DA exit signal before any close Winston did not compute; inventing fake exit signals to “make the ledger tidy”; treating Unsignaled Exit Allowance as permission to skip 2N on paper
...
**Mode D (fulfillment)**:
A portfolio fulfillment choice beside Mode C, not a fourth Plan A / Plan B / Plan C rung. The book trades the underlying long and short. Stored as `fulfillment_packaging_policy.fulfillment_mode=mode_d` with `leap_fulfillment=none`. The entry is stock only. Day orders go out on the nightly Session Order Slate. A covered call is a separate human confirm after the stock print, only on longs the operator chooses, for `floor(shares/100)` contracts. A short is never offered one. A lot without a working protective stop is naked and that is never acceptable. On Trading Strategy #341 the stop is `move_to_last_entry`: earlier lots take the newest lot's stop. A filled call does not replace that stop. Winston must match Interactive Brokers when an order is cancelled. See `docs/business-context/mode-d-ops-shell-uat.md`.
...
_Avoid_: Work Graph (that is the operating DAG in `WORK_GRAPH.md`); inventing edges not in graph.json; treating the graph as runtime proof

**Ponytail**:
Parsimonious-code discipline (YAGNI, shortest working diff) applied after the **Graphify Graph** shows what already exists, so duplicates collapse toward the owning monolith.
_Avoid_: using Ponytail to skip the graph; deleting trust-boundary or money-path checks; adding a third helper next to two copies
...
- A **WQ Shadow Portfolio** in **Wv2** tracks a **Quiver Snapshot PDF** or TXT. **DM** supplies parquet. **Quiver Skim** in **WUT** remains lab-only. Tracking is skipped by **Daily Analysis**. Ingest builds a **Monday Rebalance Plan**; **Plan Approve** locks the package. dummy_sim Confirm still books the lot. Interactive Brokers **paper**-bound WQ: each tracking Confirm **Desk Sends** one market order (exits, then rebalances, then enters); a **Working WQ Leg** waits; matched fill evidence **Accept-Fills** the journal at the print. Live Schwab write is after test cycles.
- **DM** maintains **DataCoverage** metadata that reflects parquet reality after **Reconciliation**
- **Broker Gateway** owns adapter transport and the **Winston Broker Evidence Standard** (files + API); **Wv2** owns **Confirmation Intake** match/prefill, **Desk Confirm** / later **Desk Send**, journals, **Capital Authority**, **Risk Capital**, and the operator **Fulfillment Desk** — same split pattern as **DM** (parquet + API) vs consumers
- An **Operational Portfolio** shows **Execution Mode** and a **Fulfillment Label**. dummy_sim / manual stay implied by paper/real. A bound **Adapter Binding** supplies `{Vendor} {Nickname}`. The label is who fills; it is not Capital Authority and not packaging. The **Fulfillment Label** chip links to that binding’s **Fulfillment Desk** page in Winston v2. That page has two rule layers: **Fulfillment Rituals** (binding-wide, e.g. IBKR Client Portal login / tickle) and per-OP desk rules (Confirm vs Send, Capital Authority, packaging). v1 is read-only.
...
- Agents query the **Graphify Graph** before editing; **Ponytail** then reuses or deletes rather than adding a second copy (ADR-014). The executing file remains proof; the graph is the map.
...
> **Dev:** "Should I add a second ATR helper in **Wv2**?"
> **Domain expert:** "Query the **Graphify Graph** first (`graphify path` from Wv2 ATR to DM parquet). If **DM** already owns derivatives (ADR-003), **Ponytail** means reuse, not a copy."
...
> **Dev:** "Mint is paper on Interactive Brokers DUT. Drop the paper tag and show IBKR DUT?"
> **Domain expert:** "No. Keep **Execution Mode** `paper`. Add **Fulfillment Label** `IBKR DUT070450`. Paper vs real is capital intent; the label is who fills. dummy_sim stays with no extra chip. The chip opens the Winston v2 **Fulfillment Desk** page for that **Adapter Binding** — not a Broker Gateway HTML desk."

> **Dev:** "The DUT page says Confirm = Send because WQ is on it. Does paper Mint inherit that?"
> **Domain expert:** "No. Confirm vs Send is per OP on that bind. WQ Shadow is the ADR-013 carve-out. Mint Confirm still books only. What *is* binding-wide is the **Fulfillment Ritual**: Client Portal Gateway login and tickle keep-alive until that session is solved — every OP on DUT needs a human to keep the gateway logged in."

> **Dev:** "Put the IBKR login steps on the Active OPs chip so I don't forget?"
> **Domain expert:** "No. The chip is a glance. Ops shell and WQ show **Fulfillment Label** `IBKR DUT070450` plus `needs login` if the session is dead. **DAR** and Telegram get the inline label only — who fills, not auth attention. The runbook lives on the **Fulfillment Desk** page for that binding."
...
> **Domain expert:** "No. Upload the **Quiver Snapshot PDF** (or a test TXT) on the **Quiver Tracking Desk**. That builds a **Monday Rebalance Plan**. You **Plan Approve**. dummy_sim Confirm still books the lot. On Interactive Brokers paper DUT, Confirm **Desk Sends** one market order; the journal stays a **Working WQ Leg** until the print **Accept-Fills**. The **Quiver API** stays on DM for **Alt Filings** only. This **WQ Shadow Portfolio** is not TF — no Daily Analysis signals."

> **Dev:** "Sunday I Confirm sell SPCX on the IBKR-bound WQ book — do we flatten WQ cash now?"
> **Domain expert:** "No. That click **Desk Sends** a regular-hours market sell on paper DUT (it may queue until Monday). WQ does not book cash or close the lot until matched fill evidence **Accept-Fills** at the print. Last-close is not the booked fill."

> **Dev:** "WQ is market-only — does that freeze Interactive Brokers to market when we bind paper Mint?"
> **Domain expert:** "No. Market is **Fulfillment Packaging Policy** on the **WQ Shadow Portfolio**. The same `interactive_broker_trader_api` adapter must still **Desk Send** limit and stop-market **Order Intents** for a paper Mint **Session Order Slate** (stops, exits, pyramids from EOD levels during the cash session)."

> **Dev:** "Long IBM, three of four units — is the 20-day in the market yet? Where is the stop?"
> **Domain expert:** "No. 20-day is not evaluated. All three lots stop at the 3rd purchase − 2N (GTC). The 4th is a DAY buy-stop at 3rd + the Trading Strategy pyramid step (Walnut 0.5N). When the 4th fills, every lot’s stop becomes 4th − 2N, and 20-day starts to be evaluated. When the 20-day low is above that 2N, replace the GTC with the 20-day — it may later print above the 4th purchase; all four units are then winners. That is good. The **Protective Stop Guardrail** still requires that GTC to be live on DUT every minute those lots are open."
...
- WQ “auto-execute” — resolved (Grill 2026-09-06): **Plan Approve** is Human-Gated and does not book the basket. dummy_sim Confirm still books one lot. Interactive Brokers **paper**-bound WQ: tracking Confirm = **Desk Send** of one **market** **Order Intent**; journal stays **Working WQ Leg** until matched fill **Accept-Fills** at the print. Exits before rebalances before enters. WQ market-only is **Fulfillment Packaging Policy** on that OP, not a limit on the IBKR adapter. Paper Mint/TF on the same adapter still needs stop-market / limit **Session Order Slate**. Not DA fills. Not live Schwab. Not hiding Confirm until the cash session.
...
- **Accept-fill of slate prints** — resolved (Grill 2026-09-01 Q11–Q12) then **narrowed (Grill 2026-09-09):** on Interactive Brokers **paper** DUT, a **Desk Sent** command **Accept-Fills** at the print — WQ market legs, Walnut MKT wiring proof, and Walnut stop-market slate legs (entries, pyramids, protective stops). Broker is fulfillment SoT. Dummy_sim / Manual stay Confirm. Unmatched stays attention. Whole-slate accept-fill (one click books every fill) stays aspiration. Tickle keeps an existing Client Portal session; it is not unattended login (`2026-09-06-ibkr-cpgw-unattended-session.md`). Proven 2026-09-10 on Walnut #1428 (DD / DBC / SCHZ). Poll cadence **15 minutes** (operator 2026-09-10); CPGW/Schwab push is a P1 evaluation, not a live tape. After Accept-Fill, **Fill-Driven Repark** mints a delta slate (GTC at TS N, DAY pyramid at TS step) for **Slate Approve** — not a full nightly rebuild.
- **Walnut Session Order Slate (Grill 2026-09-09, lock 2026-09-09 evening)** — stop-market only (not limits). **DAY** rebuild for entries and pyramids; **GTC + replace** for protective stops. **Slate Approve** then automatic Send of remaining ready legs (one Order Intent at a time, protective first), not a basket HTTP, not **Slate Automation** (that flag still means no Approve click). Rebuild cancels Walnut DAY tickets; never cancel-all of live protective GTC. **Slate Contest** for now: park the competing names; **Unit Heat** + first-to-touch consume cash (revisit later). Pyramid step / 2N / max lots come from the **TradingStrategy** (Walnut: 0.5N, 2N, 4). While lots < max, 20-day is **not evaluated**. After the max lot fills, all lots stop at last purchase ± 2N; 20-day is evaluated and **replaces** 2N when it has passed that level in the trade’s favor (may later print through last purchase — all units profitable). Trading rules: `docs/business-context/turtle-s2-pyramid-and-working-stop.md`. Daily Analysis today may still emit a 20-day exit while adding — recipe gap; slate rule wins at the broker.
- **Protective Stop Guardrail (Grill 2026-09-09)** — any open Position that has a **Working Stop** must have a matching protective order at the broker when the adapter can park one (IBKR stock/ETF: GTC stop-market). Evaluate continuously vs the broker book; naked lot is attention. Extra-modal (LEAP/option/related): evaluate the stop on the **signal Market**; a trigger is **HITL** to sell the related fill, not a silent option-stop unless packaging says so. Recipes with no stop (some WQ lots) are out. Never cancel-all of live protective stops.
...
- Stops — **A**: signal/default ATR + **Working Stop** on Position; human update allowed; external stop-out via **Stop-Out Reconciliation** (required position link + snapshot; warn on gap). **Protective Stop Guardrail (Grill 2026-09-09):** IBKR-bound lots with a Working Stop must have a live broker protective order; extra-modal is HITL on the related fill.
...
- **Confirm vs Send** — resolved (Grill B Q1) for Trend Following and live: **Desk Confirm** = book only; **Desk Send** = place **Order Intent** only (no auto-open Position). **Carve-out (Grill 2026-09-06):** on the Interactive Brokers **paper**-bound **WQ Shadow Portfolio**, the tracking-task Confirm click **is** **Desk Send** of that one **market** **Order Intent**; booking is **Accept-Fill** of the matched print. **Walnut paper (Grill 2026-09-09, lock evening):** wiring-proof Confirm remains one DAY MKT; the loop is **Slate Approve** then automatic **Desk Send** of remaining ready **stop-market** **Order Intents** (one at a time); DUT print **Accept-Fills**. Mint stays dummy_sim book-only. Not **Slate Automation** (no Approve click). Paper DUT `order_write` is **ADR-013**. Live Schwab remains off.
- **L1 first-ship capabilities** — resolved (Grill B Q2): **Confirmation Intake** = `auth` + `txn_read` + `order_read`; L2 `position_read` + `balance_read` are hints under **Notional Capital**, and feed **Risk Capital** under **Broker Account Capital**; `order_write` is **ADR-013** (not ADR-010 / Risk Scale).
