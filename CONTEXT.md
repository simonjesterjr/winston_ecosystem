...
**Human-Gated**:
Position open/close (and free-form desk fills) require an explicit operator desk action — ops form, ops shell, or Cromwell/MCP on a human instruction — not silent mutation by **Daily Analysis** or by inbound broker confirmation evidence alone. **Execution Mode** `real` is always human-gated for **booking entries and pyramids**. **Slate Automation** may policy-**Desk Send** the **Session Order Slate** and, in discovery, **Accept-Fill** a matched protective **Working Stop** print. On an Interactive Brokers **paper**-bound **WQ Shadow Portfolio**, the tracking-task Confirm click is the human gate that **Desk Sends** that one approved Monday-plan leg; matched fill evidence then **Accept-Fills** the journal. L1 evidence (API poll / email) may prefill or surface mismatch; other booking still requires **Desk Confirm** or **Corrective Amend**.
_Avoid_: treating draft creation as a fill, silent book-from-email or book-from-API, equating policy Send with auto-open Position, accept-fill of Trend Following entries while still in discovery, treating an IBKR bind alone as automation
...
**Accept-Fill**:
Policy that books a matched **Trade Notification** onto the **Booked Capital Spine** without a second human Confirm click. Discovery/learning under **Slate Automation**: filled protective **Working Stops** only (**Stop-Out Reconciliation**, warn on gap). On an Interactive Brokers **paper**-bound **WQ Shadow Portfolio**, a tracking Confirm that already **Desk Sent** that approved Monday-plan command **Accept-Fills** the matched print (actual qty/price) onto the journal. Other entries, pyramids, orphans, mismatches, and extra-modal packaging surprises stay Confirm. Whole-slate accept-fill is not scheduled.
_Avoid_: silent invent of unmatched prints; accept-fill of Trend Following entries in discovery; treating broker NLV movement as a booked lot; booking WQ at last-close when the DUT print is still outstanding
...
Winston v2 rules, stored on the **Operational Portfolio**, for how a desk may realize a signal (shares as-printed, round to a round lot, long-dated calls or puts, ask the human for a per-share price, **Order Intent** type, and so on). Edited in Winston v2 operations. Rule-based now; later an LLM may propose among allowed shapes and compare them (for example a long-dated-call entrance versus a calendar option spread) without a new **TradingStrategy** fingerprint. A desk (Quiver Tracking versus Trend Following operations) only supplies the default when the portfolio is created — WQ paper default is regular-hours market; Mint/TF default is the **Session Order Slate** (stop-market entry, pyramid, and protective stop). **Broker Gateway** transports the intent and classifies evidence. Packaging may differ by **Desk Action** on the same lot (entrance in long-dated calls, pyramid in shares). Split broker executions still sum to one command.
_Avoid_: putting packaging or order-type choice in the gateway; freezing IBKR to market-only because WQ is simple; a second Daily Analysis signal for the fill symbol; treating a later packaging bakeoff as a different Trading Strategy fingerprint
...
- **Accept-fill of slate prints** — resolved (Grill 2026-09-01 Q11–Q12): **B** now for Trend Following. Discovery: **Accept-Fill** matched protective **Working Stop** prints (**Stop-Out Reconciliation**, warn on gap) so Unit Heat is not a ghost lot. TF entries and pyramids stay **Desk Confirm**. **Carve-out (Grill 2026-09-06):** IBKR-paper WQ tracking Confirm already sent that command, so the matched DUT print **Accept-Fills** that journal. Whole-slate accept-fill (Q11-C) stays glossary aspiration only.
...
- **Confirm vs Send** — resolved (Grill B Q1) for Trend Following and live: **Desk Confirm** = book only; **Desk Send** = place **Order Intent** only (no auto-open Position). **Carve-out (Grill 2026-09-06):** on the Interactive Brokers **paper**-bound **WQ Shadow Portfolio**, the tracking-task Confirm click **is** **Desk Send** of that one **market** **Order Intent**; booking is **Accept-Fill** of the matched print (not last-close at the click). Mint/TF Confirm stays book-only. Paper DUT `order_write` is **ADR-013** (law accepted; implementation is a following session). Live Schwab remains off. ADR-013 and the IBKR adapter must keep limit and stop-market **Order Intents** for a later paper Mint bind (stops, exits, pyramids from EOD levels during the cash session).
