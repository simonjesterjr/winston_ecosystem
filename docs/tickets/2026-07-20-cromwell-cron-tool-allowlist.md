...
- [ ] Per-cron (or per-session-key) **allowlist** of MCP tools; calls outside the list fail closed with a short ops log line (no free-form recovery to Telegram)
- [ ] `market-snapshot-open` / `market-snapshot-hourly` allow only `wv2_market_snapshot` (plus any minimal heartbeat/status read if required)
- [ ] `eod-daily-report` allow only report-fetch path (`wv2_get_daily_activity_report` with `fetch_only`) — not `wv2_perform_daily_analysis` unless explicit operator session
- [ ] Document allowlist in `ecosystem/ai/schedule/README.md` (or equivalent) and seed path
- [ ] Smoke: force or observe one open snapshot; confirm forbidden tool call is rejected before monolith evaluate
