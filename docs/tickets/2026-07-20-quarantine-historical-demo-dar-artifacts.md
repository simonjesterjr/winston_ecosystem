...
- [ ] Inventory non-production report dates under `storage/cromwell_notifications/` and `storage/reports/`
- [ ] Move or archive demo/historical packages to a clearly named quarantine (e.g. `storage/reports/archive/demo/` or outside the agent-visible tree)
- [ ] Document that test passes write under quarantine or require `allow_historical` **and** optional `deliver_telegram`
- [ ] Confirm real EOD files (recent production dates) remain untouched
- [ ] Optional: gitignore or backup policy note if quarantine is large/binary-heavy
